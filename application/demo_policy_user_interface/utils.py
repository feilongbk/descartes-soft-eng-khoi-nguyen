...
def simulate_earthquake(policy_parameters: dict, layers: list, locations: list):
    ...
    policy = tools.earthquake_policy_module.MultiAssetEarthquakePolicy(policy_id=policy_parameters["policy_id"], policy_name=policy_parameters["name"],
                                                                         asset_locations=asset_locations,
                                                                         protection_layers=protection_layers, limit=policy_parameters["limit"],reporting_levels = reporting_levels)


    earthquake_data = usgs_api.get_earthquake_data_for_multiple_locations(asset_locations, 1.01*max([x["max_radius"] for x in layers]),0.99*min([x["min_magnitude"] for x in layers]),end_date=datetime.utcnow())
