...
from core.financial_modelling.constants.earthquake_constants import EARTH_RADIUS, DataConstantString
...
class USGSEarthQuakeScenarioGenerator(base_scenario_generator.BaseScenarioGenerator):
    def __init__(self,parameters,earthquake_dataframe:pandas.DataFrame,earth_radius = EARTH_RADIUS):
        ...
        self.asset_locations = self.parameters["asset_locations"] ## iter of tuple: latitude, longitude

        self.max_radius_tolerance_ratio = 1.01
        layer_max_radius_list = [protection_layer["max_radius"] for protection_layer in self.parameters["protection_layers"]]
        self.max_radius = max(layer_max_radius_list)
        layer_min_magnitude_list = [protection_layer["min_magnitude"] for protection_layer in self.parameters["protection_layers"]]
        self.max_radius = max(layer_max_radius_list)
        self.min_magnitude = min(layer_min_magnitude_list)

    def compute_min_distance_to_asset_locations(self,latitude,longitude):
        distance_to_all_locations = [haversine_distance.compute(latitude,longitude,*asset_location,self.earth_radius) for asset_location in self.asset_locations]
    ...
    def generate_data(self):
        ...
        ## DISTANCE
        earthquake_dataframe["min_distance"] = [self.compute_min_distance_to_asset_locations(earthquake_dataframe[DataConstantString.latitude][index_],earthquake_dataframe[DataConstantString.longitude][index_]) for index_ in earthquake_dataframe.index]
        earthquake_dataframe = earthquake_dataframe[earthquake_dataframe["min_distance"]<=self.max_radius*self.max_radius_tolerance_ratio]
        earthquake_dataframe.index = earthquake_dataframe[DataConstantString.id]
        earthquake_dataframe["datetime"] = pandas.to_datetime(earthquake_dataframe["time"]).dt.tz_convert(tz = "UTC").dt.tz_localize(None)
        earthquake_dataframe["year"] = earthquake_dataframe["datetime"].apply(lambda  x:x.year)
        earthquake_dataframe.sort_values(by =["datetime"],inplace = True)

        self.relevant_earthquake_dataframe = earthquake_dataframe

        event_split_by_years = {x:y for x, y in earthquake_dataframe.groupby ('year', as_index = False)}
        print(event_split_by_years)
        self.scenario_data = dict()
        for year in event_split_by_years:
            self.scenario_data[year] = base_scenario.BaseScenario(scenario_id=year, event_data={event_type.EventType.EARTHQUAKE: event_split_by_years[year]})
...
if __name__ == "__main__":
    ...
    parameters = dict()
    parameters["asset_locations"] = [(35.025,25.763)]
