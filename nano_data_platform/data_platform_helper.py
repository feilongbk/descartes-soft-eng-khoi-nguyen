...
def get_data_platform_sqlite_path (db_name) :
...
def create_sql_table (table_name,
                      col_data_types:dict, primary_keys:iter, delete_existing:bool = False,
    ...
    for col in col_data_types :
        if col in primary_keys :
            col_creation_list.append (Column (col, col_data_types[col], primary_key = True))
        else :

            col_creation_list.append (Column (col, col_data_types[col]))
    table_object = Table (table_name, meta, *col_creation_list)
    if delete_existing :
    ...
    return meta.create_all (sqlite_helper.get_sqlalchemy_engine (get_data_platform_sqlite_path(db_name)))
...
if __name__ == "__main__" :
    ...
    create_sql_table(table_name ="TEMPERATURE_TEST",col_data_types = {"COUNTRY":String,"DATE":Date,"HIGH":Float,"LOW":Float},primary_keys = ["COUNTRY","DATE"],delete_existing = True)
